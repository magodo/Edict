...
class OfflineScreen(Screen):
    ...
    def refer(self, word, dikt):
        ...
        if meaning:
            EdictApp.get_running_app().root.show_word(word)
        else:
...
class PersonalScreen(Screen):
    ...
    def refer(self, word, dikt):
        ...
        if meaning:
            EdictApp.get_running_app().root.show_word(word, personal_flag=True)
        else:
            modview = ModalView(auto_dismiss = True, size_hint=(.5, .1))
            modview.add_widget(Label(text = "Word %s not found!"%word))
            modview.open()
...
class EdictRoot(ScreenManager):
    ...
    def show_word(self, word, personal_flag = False):
        # save last screen
        self.last_screen = self.current
        # modify content
        self.get_screen("word").word.text = word
        if personal_flag:
            self.get_screen("word").meaning.text = self.personal_dict[word].meaning
        else:
            self.get_screen("word").meaning.text = self.offline_dict[word]
        # switch
        self.transition = FadeTransition()
        self.current = "word"
